...
describe('Selenium Data-Driven Test', function () {
    ...
    // Iterate through test data for login and task management
    testData.forEach((data, index) => {
        
        describe(`Test Case ${index + 1}: User ${data.email}`, function () {

            beforeEach(async function () {
                await driver.get('https://app.asana.com/-/login');  // Navigate to Asana login page

                // Locate and fill the email input field
                let emailInput = await driver.wait(until.elementLocated(By.css('input[type="email"]')), 15000);
                await emailInput.sendKeys(data.email);  // Use email from test data
                ...
                await passwordInput.sendKeys(data.password);  // Use password from test data
            ...
            it(`should create a new task for ${data.email}`, async function () {
                ...
                await taskName.sendKeys(data.task);  // Use task name from test data
            });

            it(`should verify task creation for ${data.email}`, async function () {
                const expectedText = data.task;  // Define the text to search for
            ...
            it(`should delete task for ${data.email}`, async function () {
